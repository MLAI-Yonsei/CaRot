...
class ClipLoss(nn.Module):
    def __init__(
        ...
        if ls:
            self.ce_loss_ls = LabelSmoothing(ls)
        else:
            self.ce_loss_ls = None
    ...
    def forward(self,
        ...
        if ground_labels is not None:
            ...
            if self.ce_loss_ls is not None:
                total_loss = (self.ce_loss_ls(logits_per_image, labels) +
                                self.ce_loss_ls(logits_per_text, labels)) / 2
            else:
                total_loss = (F.cross_entropy(logits_per_image, labels) +
                                F.cross_entropy(logits_per_text, labels)) / 2
