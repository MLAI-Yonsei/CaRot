'''
source: https://github.com/hollance/reliability-diagrams
'''
import os, sys
import numpy as np
import pandas as pd
...
def compute_calibration(true_labels, pred_labels, confidences, num_bins=10):
    ...
    for b in range(num_bins):
        ...
        if len(selected) > 0: # 
            ...
            # debug1 = true_labels[selected]
            # degub2 = pred_labels[selected]
...
def _reliability_diagram_subplot(ax, bin_data, 
    ...
    ## 
    conf_plt = ax.bar(positions, confidences, width=widths, hatch='///',
                    edgecolor='red', color=colors, alpha=0.5, linewidth=1.2, label="Confidence")
    acc_plt = ax.bar(positions, accuracies, width=widths,
                     edgecolor='black', color='blue', alpha=0.7, linewidth=1,
                     label="Accuracy")
    # acc_plt = ax.bar(positions, accuracies, width=widths,
    #                  edgecolor=colors, color=colors, alpha=0.5, linewidth=1,
    #                  label="Accuracy")
    ...
    if ece!=None:
        ...
        # ax.text(0.17, 0.8, "ECE=%.2f" % ece, color="black",fontsize=18, backgroundcolor='#F9EED8', 
        #         ha="center", va="top",
        #         bbox=dict(boxstyle='round', facecolor='#FBF0DA', alpha=0.8, ec="0.75"))
        ax.text(0.2, 0.8, "ECE=%.4f" % ece, color="black",fontsize=18, backgroundcolor='#F9EED8', 
            ha="center", va="top",
            bbox=dict(boxstyle='round', facecolor='#FBF0DA', alpha=0.8, ec="0.75"))
    ...
    ax.legend(fontsize=12,handles=[conf_plt, acc_plt])
...
def _reliability_diagram_combined(bin_data, 
                                  ...
                                  title, figsize, dpi, return_fig,ece=None):
    ...
    _reliability_diagram_subplot(ax[0], bin_data, draw_ece, draw_bin_importance, 
                                 title=title, xlabel="Confidence", ece=ece)
...
def reliability_diagram(true_labels, pred_labels, confidences, num_bins=10,
                        ...
                        figsize=(6, 6), dpi=200, return_fig=False, ece=None):
    ...
    return _reliability_diagram_combined(bin_data, draw_ece, draw_bin_importance,
                                         ...
                                         dpi=dpi, return_fig=return_fig, ece=ece)
...
def draw_reliability_diagram(y_true, y_pred, y_conf, num_bins=10, title='',ece=None):
    ...
    fig = reliability_diagram(y_true, y_pred, y_conf, num_bins=num_bins, draw_ece=True,
                            draw_bin_importance="alpha", draw_averages=False,
                            title=title, figsize=(6, 6), dpi=100, 
                            return_fig=True, ece=ece)
